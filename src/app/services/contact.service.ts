import { Injectable } from '@angular/core';
import { Contact } from '../models/contact.model';
import { BehaviorSubject } from 'rxjs';
import { StorageService } from './storage.service';
import { UtilsService } from './utils.service';

@Injectable({
  providedIn: 'root'
})

export class ContactService {
  private DEFAULT_CONTACTS = [
    {
      id: "cont1",
      name: "Nika Gerti",
      moves: [
        {
          fromUserId: "user1",
          currRate: 13504.84,
          sum: 900,
          status: "approve",
          timeStamp: this.utils.getDateAgo(2)+5
        }
      ],
      email: "nika@gmail.com",
      phone: "0542020123",
      img: "https://www.venmond.com/demo/vendroid/img/avatar/big.jpg"
    },
    {
      id: "cont2",
      name: "Adam Lavinberg",
      moves: [
        {
          fromUserId: "user1",
          currRate: 13504.83,
          sum: 500,
          status: "approve",
          timeStamp: this.utils.getDateAgo(1)
        }
      ],
      email: "adam@gmail.com",
      phone: "0547020123",
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTYmhoVBQntK9gBM5NDw5rm-YO0SVs8r2Q9Qw&usqp=CAU"
    },
    {
      id: "cont3",
      name: "Mark Gal",
      moves: [],
      email: "mark@gmail.com",
      phone: "0541320123",
      img: "https://htmlstream.com/preview/unify-v2.6.2/assets/img-temp/400x450/img5.jpg"
    },
    {
      id: "cont4",
      name: "Bill Gates",
      moves: [
        {
          fromUserId: "user1",
          currRate: 13504.84,
          sum: 807,
          status: "approve",
          timeStamp: 1604339215207
        }
      ],
      email: "bill@gmail.com",
      phone: "0542220123",
      img: "https://likewise-stage.azureedge.net/uploads/3eb6cf23-895b-45e9-b92c-5fb1b457dd04/bill-gates-profile-pic.jpg"
    },
    {
      id: "cont5",
      name: "Nicol Bari",
      moves: [],
      email: "nicol@gmail.com",
      phone: "05433320123",
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcRN1TxWGPfaC1jI69SN_CQoGz9rFgX1rRHhDQ&usqp=CAU"
    },
    {
      id: "cont6",
      name: "Magi Shaviv",
      moves: [
        {
          fromUserId: "user1",
          currRate: 13504.84,
          sum: 7005,
          status: "approve",
          timeStamp: Date.now()
        }
      ],
      email: "magi@gmail.com",
      phone: "0545620123",
      img: "https://likewisestage.blob.core.windows.net/images/adbe7230-5587-43e5-96fe-3ce863f25e88/photo.jpg"
    },
    {
      id: "cont7",
      name: "Roberto Qualo",
      moves: [
        {
          fromUserId: "user1",
          currRate: 13504.84,
          sum: 7562,
          status: "approve",
          timeStamp: this.utils.getDateAgo(3)
        }
      ],
      email: "rob@gmail.com",
      phone: "054340123",
      img: "https://cdn.fastly.picmonkey.com/contentful/h6goo9gw1hh6/2sNZtFAWOdP1lmQ33VwRN3/24e953b920a9cd0ff2e1d587742a2472/1-intro-photo-final.jpg?w=800&q=70"
    },
    {
      id: "cont8",
      name: "Gabriel Parker",
      moves: [
        {
          fromUserId: "user1",
          currRate: 13504.84,
          sum: 456,
          status: "approve",
          timeStamp: this.utils.getDateAgo(4)
        }
      ],
      email: "gabri@gmail.com",
      phone: "0542501235",
      img: "data:image/jpeg;base64,"
    },
    {
      id: "cont9",
      name: "Gaya Mishly",
      moves: [
        {
          fromUserId: "user1",
          currRate: 13504.84,
          sum: 7985,
          status: "approve",
          timeStamp: this.utils.getDateAgo(2)
        }
      ],
      "email": "gaya@gmail.com",
      "phone": "0543012355",
      "img": "https://static.jobscan.co/blog/uploads/linkedin-profile-picture.jpg"
    },
    {
      id: "cont10",
      name: "Sara Parker",
      moves: [],
      email: "sara@gmail.com",
      phone: "0546020123",
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcRIfBrTzjblqtKxXgRcDV63uJjYq29SC9WvBg&usqp=CAU"
    }
  ]
  private _contacts$ = new BehaviorSubject<any>([])
  public contacts$ = this._contacts$.asObservable()
  private _currContact$ = new BehaviorSubject<any>([])
  public currContact$ = this._currContact$.asObservable()
  private STORAGE_KEY = 'CONTACT'
  constructor(
    private storage: StorageService,
    private utils: UtilsService
  ) { }

  getContacts() {
    const contacts: Contact[] = this.storage.load(this.STORAGE_KEY) ||
      this.storage.save(this.STORAGE_KEY, this.DEFAULT_CONTACTS)
    this._contacts$.next(contacts)
  }

  getContact(id: string) {
    const contact = this.storage.load(this.STORAGE_KEY).
      filter(contact => contact.id === id)
    this._currContact$.next(contact[0])
    return id
  }

  updateContact(contact: Contact) {
    const newContacts = this.storage.load(this.STORAGE_KEY).
      map(_contact => {
        if (contact.id === _contact.id) return contact
        return _contact
      })
    this._currContact$.next(contact)
    this._contacts$.next(this.storage.save(this.STORAGE_KEY, newContacts))
    return contact
  }

  addContact(contact) {
    const newContact = { ...contact }
    newContact.id = this.utils.genId()
    let newContacts = this.storage.load(this.STORAGE_KEY)
    newContacts.push(newContact)
    this._contacts$.next(this.storage.save(this.STORAGE_KEY, newContacts))
    return contact
  }

  deleteContact(contact: Contact) {
    let newContacts = this.storage.load(this.STORAGE_KEY).
      filter(_contact => _contact.id !== contact.id)
    this._contacts$.next(this.storage.save(this.STORAGE_KEY, newContacts))
    return contact
  }

  getMove(fromUserId, currRate, sum) {
    return {
      fromUserId,
      currRate,
      sum,
      status: 'approve',
      timeStamp: Date.now()
    }
  }
}


