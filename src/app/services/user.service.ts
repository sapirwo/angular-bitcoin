import { Injectable } from '@angular/core';
import { User } from '../models/user.model';
import { BehaviorSubject } from 'rxjs';
import { UtilsService } from './utils.service';
import { StorageService } from './storage.service';
import { Url } from 'url';

@Injectable({
  providedIn: 'root'
})
export class UserService {
  private DEFAULT_USERS = [
    {
      id: "user1",
      name: "David Gordon",
      coins: 100,
      usd: 9842009,
      moves: [
        {
          toContact: {
            id: "cont1",
            name: "Nika Gerti",
            img: "https://www.venmond.com/demo/vendroid/img/avatar/big.jpg"
          },
          currRate: 13504.84,
          sum: 900,
          status: "approve",
          timeStamp: this.utils.getDateAgo(2)+5
        },
        {
          toContact: {
            id: "cont2",
            name: "Adam Lavinberg",
            img: "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTYmhoVBQntK9gBM5NDw5rm-YO0SVs8r2Q9Qw&usqp=CAU"
          },
          currRate: 13504.83,
          sum: 500,
          status: "approve",
          timeStamp: this.utils.getDateAgo(1)
        },
        {
          toContact: {
            id: "cont4",
            name: "Bill Gates",
            img: "https://likewise-stage.azureedge.net/uploads/3eb6cf23-895b-45e9-b92c-5fb1b457dd04/bill-gates-profile-pic.jpg"
          },
          currRate: 13504.84,
          sum: 807,
          status: "approve",
          timeStamp: 1604339215207
        },
        {
          toContact: {
            id: "cont6",
            name: "Magi Shaviv",
            img: "https://likewisestage.blob.core.windows.net/images/adbe7230-5587-43e5-96fe-3ce863f25e88/photo.jpg"
          },
          currRate: 13504.84,
          sum: 7005,
          status: "approve",
          timeStamp: Date.now()
        },
        {
          toContact: {
            id: "cont7",
            name: "Roberto Qualo",
            img: "https://cdn.fastly.picmonkey.com/contentful/h6goo9gw1hh6/2sNZtFAWOdP1lmQ33VwRN3/24e953b920a9cd0ff2e1d587742a2472/1-intro-photo-final.jpg?w=800&q=70"
          },
          currRate: 13504.84,
          sum: 7562,
          status: "approve",
          timeStamp: this.utils.getDateAgo(3)
        },
        {
          toContact: {
            id: "cont8",
            name: "Gabriel Parker",
            img: "data:image/jpeg;base64,"
          },
          currRate: 13504.84,
          sum: 456,
          status: "approve",
          timeStamp: this.utils.getDateAgo(4)
        },
        {
          toContact: {
            id: "cont9",
            name: "Gaya Mishly",
            img: "https://static.jobscan.co/blog/uploads/linkedin-profile-picture.jpg"
          },
          currRate: 13504.84,
          sum: 7985,
          status: "approve",
          timeStamp: this.utils.getDateAgo(2)
        }
      ]
    }
  ]
  private _users$ = new BehaviorSubject<any>([])
  public users$ = this._users$.asObservable()
  private _currUser$ = new BehaviorSubject<any>([])
  public currUser$ = this._currUser$.asObservable()
  private STORAGE_KEY = 'USER'
  constructor(
    private storage: StorageService,
    private utils: UtilsService
  ) { }

  getUsers() {
    const users: User[] = this.storage.load(this.STORAGE_KEY) ||
      this.storage.save(this.STORAGE_KEY, this.DEFAULT_USERS)
    this._users$.next(users)
  }

  getUser(id: string) {
    let user = this.storage.load(this.STORAGE_KEY) ||
      this.storage.save(this.STORAGE_KEY, this.DEFAULT_USERS)
    user = user.filter(user => user.id === id)
    this._currUser$.next(user[0])
    return id
  }

  updateUser(user: User) {
    const newUsers = this.storage.load(this.STORAGE_KEY).
      map(_user => {
        if (user.id === _user.id) return user
        return _user
      })
    this._currUser$.next(user)
    this._users$.next(this.storage.save(this.STORAGE_KEY, newUsers))
    return user
  }

  addUser(user: User) {
    const newUser = { ...user }
    newUser.id = this.utils.genId()
    let newUsers = this.storage.load(this.STORAGE_KEY)
    newUsers.push(newUser)
    this._users$.next(this.storage.save(this.STORAGE_KEY, newUsers))
    return user
  }

  deleteUser(user: User) {
    let newUsers = this.storage.load(this.STORAGE_KEY).
      filter(_user => _user.id !== user.id)
    this._users$.next(this.storage.save(this.STORAGE_KEY, newUsers))
    return user
  }

  getMove(id: string, name: string, img: Url, currRate: number, sum: number) {
    return {
      toContact: {
        id,
        name,
        img,
      },
      currRate,
      sum,
      status: "approve",
      timeStamp: Date.now()
    }
  }
}
